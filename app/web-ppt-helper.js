/* Narrow, consent-gated window messaging. Never accept arbitrary websites or file paths. */
...
function initializeWebPptHelper() {
  ...
  const opener=window.opener;let approved=false,busy=false;
  ...
  allow.onclick=()=>{approved=true;allow.hidden=true;disconnect.hidden=false;status.textContent='已連接。可以回網頁繼續畫圖、複製；請保留此視窗。';reply('approved');};
  ...
  window.addEventListener('message',async event=>{
    ...
    if(event.source!==opener||event.origin!==origin||data?.kind!=='skechu-ppt'||data.channel!==channel||data.type!=='copy')return;
    if(!approved||busy){reply('result',{id:data.id,result:{ok:false,error:busy?'本機 PowerPoint 忙碌中':'請先允許連接'}});return;}
    busy=true;bar.hidden=false;bar.value=0;
    try{
      const payload=validWebPptPayload(data.body);
      status.textContent='正在建立原生物件…';
      const response=await fetch('/copy',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(payload)});
      ...
      const accept=line=>{if(!line.trim())return;const event=JSON.parse(line);if(event.type==='progress'){bar.value=event.percent||0;status.textContent=event.stage+'（'+(event.percent||0)+'%）';reply('progress',{id:data.id,event});}else if(event.type==='result')result=event;};
      ...
      status.textContent='已複製 '+result.count+' 個可編輯物件。切到 PowerPoint 按 Ctrl+V；需要時取消群組即可逐一編輯。';reply('result',{id:data.id,result});
    }catch(error){status.textContent='未確認成功：'+(error.message||error);reply('result',{id:data.id,result:{ok:false,error:String(error.message||error)}});}
    finally{busy=false;bar.hidden=true;}
  });
  reply('ready');
}
