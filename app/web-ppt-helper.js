...
function initializeWebPptHelper() {
  ...
  const capabilities=['prepare','cache-contexts'];
  ...
  window.addEventListener('message',async event=>{
    ...
    if(event.source!==opener||event.origin!==origin||data?.kind!=='skechu-ppt'||data.channel!==channel||!['copy','prepare'].includes(data.type))return;
