...
- **Browser-local image HD:** a standalone **圖片高清** action beside **新增底圖** offers 2× / 4× anime super-resolution, before/after comparison, PNG download, and a new reference page without replacing the original. The approximately 4 MB model/runtime loads only on explicit use; images stay on-device. GPU support is required for AI mode, with a clearly labelled non-AI alternative. [Usage, limits and model licenses](docs/browser-hd.md).
