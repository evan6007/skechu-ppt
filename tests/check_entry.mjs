...
assert.ok(browserCapture.includes('const autoCloseFrames=28')&&browserCapture.includes('step/(autoCloseFrames-1)*359'),'Closing the auto trace preview must be sampled densely enough to show the real shrink-back motion');
assert.ok(browserCapture.includes("await waitFor(`items.some(item=>item.autoTrace)`)")&&browserCapture.includes('clearSelectionState(false);render()'),'The auto trace close capture must wait for the delayed apply and hide premature anchor selection');
...
assert.ok(gifBuilder.includes('freshen_orange')&&gifBuilder.includes('colors=256'),'Auto trace GIF must preserve a clean orange accent with a high-color palette');
assert.ok(gifBuilder.includes('tween(auto_opening[-1], auto_preview, 6)')&&gifBuilder.includes('tween(auto_preview, auto_closing[0], 6)')&&gifBuilder.includes('tween(auto_closing[-1], auto_applied_clean, 6)'),'Auto trace GIF must ease the modal chrome and traced lines in and out without a hard cut');
