...
const readme=read('README.md'),asset='assets/brand/open-web-cta.svg',svg=read(asset),gifBuilder=read('scripts/build_showcase_gifs.py'),pptCapture=read('scripts/capture_powerpoint_demo.py');
...
assert.ok(gifBuilder.includes('fixed_view')&&gifBuilder.includes('trace_view')&&gifBuilder.includes('aero_arrow.cur')&&gifBuilder.includes('ppt_pasted'),'Feature demos must use fixed views, a normal system cursor, real magnetic snapping and real PowerPoint state changes');
