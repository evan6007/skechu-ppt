...
assert.ok(browserCapture.includes('const coarse=[]')&&browserCapture.includes('referenceBoundaryRoute(before,after)')&&browserCapture.includes('track=Array.from({length:121}')&&browserCapture.includes('const edge=track[0]')&&browserCapture.includes('motionOrder = Array.from')&&browserCapture.includes('centralGuideY')&&browserCapture.includes('traceSnapTarget.y-18')&&gifBuilder.includes('crop_width = min(420')&&!gifBuilder.includes('anchor=anchor')&&gifBuilder.includes('橘點吸住線條滑動'),'Magnetic tracing demo must show one native orange marker gliding slowly in one direction along one densely sampled central curve');
