...
assert.ok(pptCapture.includes('rank_by_index')&&pptCapture.includes('for step in range(49)')&&pptCapture.includes('display_scale'),'PowerPoint color regions must expand clockwise in a smooth sequence around the slide, with oversized fills reduced for visibility');
