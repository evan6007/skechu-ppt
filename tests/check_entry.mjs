...
assert.ok(gifBuilder.includes('auto_applied_aligned = reposition_patch')&&gifBuilder.includes('auto_align_motion = move_patch_sequence')&&gifBuilder.includes('tween(auto_preview, auto_applied_aligned, 6)')&&!gifBuilder.includes('tween(auto_preview, auto_applied, 4)'),'Auto trace modal and applied brain must overlap before one solid brain moves into its canvas position');
