...
assert.ok(gifBuilder.includes('auto_preview_close_bounds = (0.452, 0.574, 0.682, 0.780)')&&gifBuilder.includes('zoom_sequence(auto_preview_source, auto_bounds, auto_preview_close_bounds, 12)')&&gifBuilder.includes('"複雜線條完整描出", auto_preview_close')&&gifBuilder.includes('range(22)'),'Auto trace demo must center, enlarge and briefly hold the raised cerebellum tracing');
assert.ok(gifBuilder.includes('auto_canvas_match_bounds = (0.166, 0.134, 0.813, 0.763)')&&gifBuilder.includes('auto_preview_match_bounds = (0.2124106, 0.3071004, 0.7901768, 0.8687928)')&&gifBuilder.includes('auto_preview_match_angle = 0.5727505')&&gifBuilder.includes('zoom_sequence(auto_imported_source, auto_bounds, auto_canvas_match_bounds, 8)')&&gifBuilder.includes('tween(auto_imported_match, auto_preview_match, 5)')&&gifBuilder.includes('auto_bounds, auto_preview_match_bounds, 8, 0, auto_preview_match_angle')&&gifBuilder.includes('zoom_sequence(auto_applied_source, auto_canvas_match_bounds, auto_bounds, 8)'),'Auto trace modal must use image-registered position, scale and angle to zoom from the source brain into its preview and reverse back to the unchanged canvas brain');
