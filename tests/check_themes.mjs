...
for(const marker of ['--surface','--canvas-desk','--glass-surface','--range-track','.stage-shell{position:relative','data-ui-theme="graphite"','theme-controls.js?v=46-canvas-shapes','theme-controls.css?v=51-mobile-workspace','fill-opacity:1','.action-menu-popover{position:fixed','input[type="range"]::-webkit-slider-runnable-track','grid-template-rows:48px 58px minmax(0,1fr)','height:min(48dvh,420px)'])assert.ok(css.includes(marker)||html.includes(marker),'Missing theme marker '+marker);
