...
for(const marker of ['--surface','--canvas-desk','--glass-surface','--range-track','.stage-shell{position:relative','data-ui-theme="graphite"','theme-controls.js?v=46-canvas-shapes','theme-controls.css?v=53-page-canvas-style','--paper-fill:#fff','fill-opacity:.3','.action-menu-popover{position:fixed','input[type="range"]::-webkit-slider-runnable-track','grid-template-rows:48px 58px minmax(0,1fr)','height:min(48dvh,420px)'])assert.ok(css.includes(marker)||html.includes(marker),'Missing theme marker '+marker);
...
assert.ok(css.includes('.topbar{grid-row:2')&&css.includes('.workspace{grid-row:3}'),'Mobile tools must stay above the workspace instead of under the browser toolbar');
assert.ok(html.includes("function tracePenStrokeStyle(){return{color:'#123f8c',width:3.5}}")&&html.includes('migrateTraceStrokeWidths()'),'New and existing traced lines must remain clearly visible on the opaque paper canvas');
