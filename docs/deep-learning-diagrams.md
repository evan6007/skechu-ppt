...
The gallery contains seven starters, including new Latent Diffusion and Vision Transformer figures. The depth starter uses a 1560 × 900 canvas with matching reflective-object and schematic depth scenes, a constant-width ViT stack, expanding DPT decoder, multi-scale fusion and a candidate gated residual adapter. Its main labels are 24–32 px. Each template creates a **new page**.

The gallery offers 24 reusable pieces. Ten additions cover contracting encoder and expanding decoder trapezoids, an hourglass U-Net, a U-shaped feature pyramid, a repeated Transformer tower, a forward noise sequence, a denoising loop, a cross-attention bridge, a latent bottleneck and paired RGB/depth scenes. The original fourteen detailed components remain available. All IDs are discoverable through `list_diagram_components` and insertable with `create_diagram_component` in MCP. Inserting a component is one Undo step.

Choose **圖中文字 → 短標籤 / 無文字** before inserting a template or component. Both `createTemplate(id, {labels:'none'})` and `createComponent(id, {labels:'none'})` preserve geometry and omit text objects. MCP component creation accepts the same optional `labels` enum. Existing projects are not rewritten.
...
The detailed Transformer component follows the pre-norm structure in [ViT](https://arxiv.org/abs/2010.11929); the compact tower omits normalization and position encoding for clarity. The decoder summarizes reassembly and fusion from [DPT](https://github.com/isl-org/DPT). The [diffusion starter](https://huggingface.co/blog/annotated-diffusion) abstracts the denoiser, scheduler step and repeated sampling loop; its noise cells and decoded scene are illustrations. Layer taps, dimensions and head variants depend on the chosen implementation. The residual adapter is a candidate design, not a published or validated method. Its two expanding heads include projection and resizing, then predict a sigmoid gate and signed residual. Their product is added to baseline depth. These compact diagrams omit implementation details; they do not establish improved performance on reflections.
