...
網頁版的 **專案與匯出 → 匯出選取 PPTX（跨平台原生物件）**，可以直接在瀏覽器產生 `.pptx`，不需要安裝 Windows 連接元件、登入或上傳圖稿。
...
Windows 連接元件的 Ctrl+C 直接複製流程保持不變。非 Windows 複製向量時，會說明目前需經過 PPTX，不會顯示 Windows 安裝包、宣稱已寫入剪貼簿，或自動下載。單獨選取一張底圖／圖片時，各平台會直接嘗試寫入瀏覽器圖片剪貼簿，不需要 Windows 元件；不支援或權限被拒時會說明原因並提供 PNG 下載。
...
- v95 起，明確選取的底圖與一般圖片會保留為獨立圖片物件，與原生曲線、文字、漸層一起匯出。底圖的透明度和裁切會保留在圖片像素中；線條不會跟著變成圖片。隱藏物件不輸出。LaTeX 仍會阻止整次匯出，不會偷偷省略。
- 單張來源圖片限 1,600 萬像素、最長邊 8,192 px；混合輸出的內嵌圖片總量約限 9 MB，超過會提示分批。Windows 直接複製混合底圖需要支援 `inline-image-v1` 的新版連接元件（v0.1.8）；舊版會提示更新，或使用免安裝 PPTX 匯出。
