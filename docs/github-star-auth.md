...
Production configuration uses [Skechu-PPT Star](https://github.com/apps/skechu-ppt-star) and the dedicated `skechu-github-star.evan6007.workers.dev` service. The following steps document re-deployment or self-hosting; no secret values are stored here.
...
2. Request only **Account permissions → Starring → Read and write**, with implicit/public Metadata read access. Leave repository contents, email, organization permissions and webhooks off. Keep user-token expiration enabled; device flow is not needed. Make it public so other users can authorize it. Users authorize the app; no installation into their repositories is needed for this public-repository feature.
